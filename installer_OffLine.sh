...
if [[ "$check_amrif" == "$firma2" ]]; then  
    ...
    info "${YELLOW}Changing to use FIRST STREAM= AAC 5.1 384kbps (or AC3 5.1 512kbps), SECOND STREAM= MP3 2.0 192kbps in VIDEO-STATION." >> $logfile
    
    # Modifier les configurations pour AAC en 5.1 avec un tampon plus grand et une qualité réduite
    sed -i 's/args2vs+=("-c:a:0" "$1" "-c:a:1" "libfdk_aac")/args2vs+=("-c:a:0" "libfdk_aac" "-c:a:1" "$1" "-max_delay" "40000000" "-bufsize" "16000000")/gi' ${cp_bin_path}/ffmpeg41 2>> $logfile
    sed -i 's/args2vs+=("-ac:1" "$1" "-ac:2" "6")/args2vs+=("-ac:1" "6" "-ac:2" "$1")/gi' ${cp_bin_path}/ffmpeg41 2>> $logfile
    sed -i 's/args2vs+=("-b:a:0" "256k" "-b:a:1" "512k")/args2vs+=("-b:a:0" "384k" "-b:a:1" "192k" "-max_delay" "40000000" "-bufsize" "16000000")/gi' ${cp_bin_path}/ffmpeg41 2>> $logfile

    # Modifier les configurations pour AC3 en 5.1 avec un tampon plus grand et une qualité réduite
    sed -i 's/args2vs+=("-c:a:0" "$1" "-c:a:1" "ac3")/args2vs+=("-c:a:0" "ac3" "-c:a:1" "$1" "-max_delay" "40000000" "-bufsize" "16000000")/gi' ${cp_bin_path}/ffmpeg41 2>> $logfile
    sed -i 's/args2vs+=("-ac:1" "$1" "-ac:2" "")/args2vs+=("-ac:1" "" "-ac:2" "$1")/gi' ${cp_bin_path}/ffmpeg41 2>> $logfile
    sed -i 's/args2vs+=("-b:a:0" "256k" "-b:a:1" "640k")/args2vs+=("-b:a:0" "512k" "-b:a:1" "192k" "-max_delay" "40000000" "-bufsize" "16000000")/gi' ${cp_bin_path}/ffmpeg41 2>> $logfile
