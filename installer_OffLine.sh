...
repo_url="https://raw.githubusercontent.com/Tom21200/Wrapper_VideoStation"
setup="start"
...
if [[ "$check_amrif" == "$firma2" ]]; then  
    ...
    info "${YELLOW}Changing to use FIRST STREAM= AAC 2.0 96kbps (or AC3 2.0 128kbps), SECOND STREAM= MP3 2.0 96kbps in VIDEO-STATION." >> $logfile

    # Utiliser un tampon plus grand et forcer le stéréo pour AAC avec une qualité plus basse
    sed -i 's/args2vs+=("-c:a:0" "libfdk_aac" "-c:a:1" "$1")/args2vs+=("-c:a:0" "aac" "-c:a:1" "$1" "-ac:1" "2" "-max_delay" "10000000" "-bufsize" "4000000")/gi' ${cp_bin_path}/ffmpeg41 2>> $logfile
    sed -i 's/args2vs+=("-b:a:0" "512k" "-b:a:1" "256k")/args2vs+=("-b:a:0" "96k" "-b:a:1" "96k" "-ac:1" "2" "-max_delay" "10000000" "-bufsize" "4000000")/gi' ${cp_bin_path}/ffmpeg41 2>> $logfile

    # Utiliser un tampon plus grand et forcer le stéréo pour AC3 avec une qualité plus basse
    sed -i 's/args2vs+=("-c:a:0" "ac3" "-c:a:1" "$1")/args2vs+=("-c:a:0" "ac3" "-c:a:1" "$1" "-ac:1" "2" "-max_delay" "10000000" "-bufsize" "4000000")/gi' ${cp_bin_path}/ffmpeg41 2>> $logfile
    sed -i 's/args2vs+=("-b:a:0" "640k" "-b:a:1" "256k")/args2vs+=("-b:a:0" "128k" "-b:a:1" "96k" "-ac:1" "2" "-max_delay" "10000000" "-bufsize" "4000000")/gi' ${cp_bin_path}/ffmpeg41 2>> $logfile
...
function crackmenu() {
 ...
 text_crackmenu_2=("RETURN to THE FUCKING MAIN menu." "VOLVER al MENU Principal." "VOLTAR ao MENU Principal." "RETOUR au MENU Principal." "ZURÜCK zum Hauptmenü." "INDIETRO al menù principale.")
...
function language() {
 ...
 text_language_2=("RETURN to THE FUCKING MAIN menu." "VOLVER al MENU Principal." "VOLTAR ao MENU Principal." "RETOUR au MENU Principal." "ZURÜCK zum Hauptmenü." "INDIETRO al menù principale.")
...
text_configura_21=(" ( Z ) RETURN to THE FUCKING MAIN menu." " ( Z ) VOLVER al menú PRINCIPAL." " ( Z ) VOLTAR ao menu PRINCIPAL." " ( Z ) RETOUR au menu PRINCIPAL." " ( Z ) ZURÜCK zum HAUPTMENÜ." " ( Z ) TORNA al menu PRINCIPALE.")
