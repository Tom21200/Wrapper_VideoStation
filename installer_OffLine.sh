...
if [[ "$check_amrif" == "$firma2" ]]; then  
    ...
    info "${YELLOW}Changing to use FIRST STREAM= AAC 5.1 448kbps (or AC3 5.1 576kbps), SECOND STREAM= MP3 2.0 256kbps in VIDEO-STATION." >> $logfile

    # Utiliser un tampon encore plus grand et ajuster légèrement les bitrates pour AAC
    sed -i 's/args2vs+=("-c:a:0" "libfdk_aac" "-c:a:1" "$1")/args2vs+=("-c:a:0" "aac" "-c:a:1" "$1" "-max_delay" "20000000" "-bufsize" "8000000")/gi' ${cp_bin_path}/ffmpeg41 2>> $logfile
    sed -i 's/args2vs+=("-b:a:0" "512k" "-b:a:1" "256k")/args2vs+=("-b:a:0" "448k" "-b:a:1" "256k" "-max_delay" "20000000" "-bufsize" "8000000")/gi' ${cp_bin_path}/ffmpeg41 2>> $logfile

    # Utiliser un tampon encore plus grand et ajuster légèrement les bitrates pour AC3
    sed -i 's/args2vs+=("-c:a:0" "ac3" "-c:a:1" "$1")/args2vs+=("-c:a:0" "ac3" "-c:a:1" "$1" "-max_delay" "20000000" "-bufsize" "8000000")/gi' ${cp_bin_path}/ffmpeg41 2>> $logfile
    sed -i 's/args2vs+=("-b:a:0" "640k" "-b:a:1" "256k")/args2vs+=("-b:a:0" "576k" "-b:a:1" "256k" "-max_delay" "20000000" "-bufsize" "8000000")/gi' ${cp_bin_path}/ffmpeg41 2>> $logfile
