...
if [[ "$check_amrif" == "$firma2" ]]; then  
    ...
    # Modifier les configurations pour ajouter un cache pour les flux audio
    sed -i 's/args2vs+=("-c:a:0" "libfdk_aac" "-c:a:1" "$1")/args2vs+=("-c:a:0" "libfdk_aac" "-c:a:1" "$1" "-max_delay" "5000000" "-bufsize" "2000000")/gi' ${cp_bin_path}/ffmpeg41 2>> $logfile
    sed -i 's/args2vs+=("-b:a:0" "512k" "-b:a:1" "256k")/args2vs+=("-b:a:0" "512k" "-b:a:1" "256k" "-max_delay" "5000000" "-bufsize" "2000000")/gi' ${cp_bin_path}/ffmpeg41 2>> $logfile

    sed -i 's/args2vs+=("-c:a:0" "ac3" "-c:a:1" "$1")/args2vs+=("-c:a:0" "ac3" "-c:a:1" "$1" "-max_delay" "5000000" "-bufsize" "2000000")/gi' ${cp_bin_path}/ffmpeg41 2>> $logfile
    sed -i 's/args2vs+=("-b:a:0" "640k" "-b:a:1" "256k")/args2vs+=("-b:a:0" "640k" "-b:a:1" "256k" "-max_delay" "5000000" "-bufsize" "2000000")/gi' ${cp_bin_path}/ffmpeg41 2>> $logfile
